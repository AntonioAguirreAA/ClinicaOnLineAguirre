...
export class HistoriaClinicaComponent implements OnInit {
  ...
  const turnos = data!.map(row => new Turno(
    row.id,
  ...
  /* 3. filtrar: sólo los que tienen historia clínica --- */
  const finalizados = turnos.filter(t =>
    Array.isArray(t.historiaClinica) && t.historiaClinica.length > 0
  );

  /* 4. completar nombre del especialista si falta ------ */
